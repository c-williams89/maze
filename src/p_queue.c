...
pqueue_t *pqueue_create(int capacity)
{
	...
	if (capacity < 1) {
		fprintf(stderr,
			"pqueue_create: Invalid argument. Size must be integer greater than 0.\n");
		goto CREATE_EXIT;
	}
	...
	if (!pq) {
		perror
		    ("pqueueu_create: Unable to allocate memory for priority queue\n");
		errno = 0;
		goto CREATE_EXIT;
	}
	...
	if (!pq->heap) {
		perror
		    ("pqueue_create: Unable to allocate memory for target capacity\n");
		errno = 0;
		free(pq);
		pq = NULL;
		goto CREATE_EXIT;
...
static void heapify_down(uint32_t position, uint64_t size, node_t * heap)
{
	...
	uint32_t swap_idx =
	    heap[l_child].priority < heap[r_child].priority ? l_child : r_child;
